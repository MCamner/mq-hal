...
echo "[2/6] sample critique works"
"$ROOT/bin/mq-hal" critic --sample >/dev/null

echo "[3/6] sample output contains expected sections"
out="$("$ROOT/bin/mq-hal" critic --sample)"
...
r = subprocess.run(
    ["$ROOT/bin/mq-hal", "critic", "--sample", "--json"],
    capture_output=True, text=True,
)
assert r.returncode == 0, f"exit {r.returncode}: {r.stderr}"
d = json.loads(r.stdout)
for key in ("goal", "risk", "verdict", "findings",
...
echo "[5/6] dangerous plan returns FAIL verdict"
python3 - <<EOF
import json, sys, os
...
echo "[6/6] clean plan returns PASS or REVIEW"
python3 - <<EOF
import json, sys
sys.path.insert(0, '$ROOT/scripts')
import critic
